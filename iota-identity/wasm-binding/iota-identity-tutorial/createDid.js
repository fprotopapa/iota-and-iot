...
const { Document, KeyType, Client, Config, KeyPair, VerificationMethod } = require("@iota/identity-wasm/node");
...
async function createDid(subjectName) {
    ...
    const doc = new Document(keyPair, CLIENT_CONFIG.network.toString())
    const verifiMethod = new VerificationMethod(keyPair, "sign-0");
    //Sign the DID Document with the generated key
    doc.signSelf(keyPair, "sign-0");
    ...
    const messageId = await client.publishDocument(doc);
    ...
    storeWeakholdObject(subjectName, doc, messageId, keyPair, null);

    //Return the results
    return {keyPair, doc, messageId};
}
